# Certifications

Welcome to my Certifications repository!  
This repository showcases my completed courses and certifications, demonstrating my skills and learning journey in **Python programming**, **Generative AI**, **Responsible AI**, and **Large Language Models (LLMs)**.

---

## 🎯 Key Areas

- **Python Programming:** Core concepts, object-oriented programming (OOP), exception handling (`try-except`), and libraries such as **NumPy**, **Pandas**, **Matplotlib**, **Seaborn**, and **Regular Expressions**.  
- **Generative AI (Google Gemini):** Prompt engineering, AI workflows, and responsible AI practices.  
- **Responsible AI:** Ethical AI usage, fairness, transparency, privacy, and best practices.  
- **Large Language Models (LLMs):** Understanding model capabilities, prompt design, and real-world applications.

---

## 📜 Certificate List

| Course / Certification                   | Platform       | Certificate / Link |
|----------------------------------------|----------------|------------------|
| Python Basics                           | Scholar Hat    | Certificate ID: **JTH7291125** |
| Python Programming                      | STEAM UP       | N/A |
| Google Gemini AI Certified Student      | Google         | [View Certificate](https://cdn.qwiklabs.com/VF6eQ1bV8SfqKBpu8AVD%2F4G93sg9wJQ0wWe9CTc%2FJ%2F4%3D) |
| Introduction to GenAI                   | Qwiklabs       | [View Certificate](https://cdn.qwiklabs.com/VF6eQ1bV8SfqKBpu8AVD%2F4G93sg9wJQ0wWe9CTc%2FJ%2F4%3D) |
| Responsible AI                          | Qwiklabs       | [View Certificate](https://cdn.qwiklabs.com/6tckOOT%2FbCYrVDwPWyxmWFjvnl1Sv6yO3btQsuE%2F%2FI4%3D) |
| Introduction to Large Language Models   | Qwiklabs       | [View Certificate](https://cdn.qwiklabs.com/licxd8JDS5%2FvJ3%2FcXDcU2Wmyj8Ii8Old17CuG49CW%2BM%3D) |

---

## 📌 How to Use

Each folder or link contains the respective certificate files. This repository is meant to **showcase my skills, learning progress, and professional development** in programming and AI.

---

## 📬 Contact

- **Email:** btwaliraza110@gmail.com  
- **LinkedIn:** [Ali Raza](https://www.linkedin.com/in/ali-r-a8a144391)

